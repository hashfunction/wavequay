...
option(AU_TRIEFLOW_DISTRIBUTION "Build the offline WaveWeft distribution" OFF)
if(AU_TRIEFLOW_DISTRIBUTION)
    foreach(_option IN ITEMS AU_BUILD_CLOUD_AUDIOCOM AU_BUILD_USAGEINFO_MODULE
        ...
        if(DEFINED ${_option} AND ${_option})
            message(FATAL_ERROR "WaveWeft forbids ${_option}=ON; use a clean offline build directory")
        endif()
        set(${_option} OFF CACHE BOOL "Disabled for WaveWeft" FORCE)
    endforeach()
    foreach(_url IN ITEMS MUSE_MODULE_DIAGNOSTICS_CRASHREPORT_URL CRASH_REPORT_URL SENTRY_DSN_KEY SENTRY_HOST SENTRY_PROJECT)
        if(DEFINED ${_url} AND NOT "${${_url}}" STREQUAL "")
            message(FATAL_ERROR "WaveWeft forbids ${_url}")
        endif()
        set(${_url} "" CACHE STRING "Disabled for WaveWeft" FORCE)
    endforeach()
endif()
