...
$result=[ordered]@{schemaVersion=1;sourceCommit=$commit;runId=$env:GITHUB_RUN_ID;runAttempt=$env:GITHUB_RUN_ATTEMPT;
...
try {
    foreach($mode in @('qualification','store')) {
        ...
        if($receipt.identityMode -cne $mode -or $receipt.sourceCommit -cne $commit -or $receipt.installation_qualification_passed -ne $true){throw 'Installed receipt mode/source/completion differs'}
...
Write-Output 'PASS: two separate installed consumer lifecycles. Source publication and unsigned Store export remain separate gates.'
