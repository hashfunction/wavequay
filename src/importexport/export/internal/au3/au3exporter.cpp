...
std::optional<ExportRecipeFormat> Au3Exporter::recipeFormat(const std::string& format,
                                                          const std::map<int, OptionValue>& parameters) const
{
    for (auto [plugin, index] : ExportPluginRegistry::Get()) {
        ...
        for (const auto& [id,value] : parameters) {
            ::ExportValue current;
            if (!editor->GetValue(id,current) || current.index() != value.index()) return std::nullopt;
            // Untrusted JSON must never reach a typed encoder setter before
            // its descriptor and variant type have been checked.
            bool found = false;
            for (int i = 0; i < editor->GetOptionsCount(); ++i) {
                ::ExportOption descriptor;
                if (!editor->GetOption(i, descriptor) || descriptor.id != id) continue;
                if ((descriptor.flags & ExportOption::ReadOnly) && current != value) return std::nullopt;
                found = true;
                break;
            }
            if (!found || (current != value && !editor->SetValue(id,value))) return std::nullopt;
        }
        ExportRecipeFormat result;
        result.id=format; result.maxChannels=plugin->GetFormatInfo(index).maxChannels;
        result.sampleRates=editor->GetSampleRateList();
        for (int i=0;i<editor->GetOptionsCount();++i) {
            ::ExportOption option; ::ExportValue value;
            if (!editor->GetOption(i,option) || !editor->GetValue(option.id,value)) continue;
            std::vector<std::string> names;
            for (const auto& name:option.names) names.push_back(name.translated().toStdString());
