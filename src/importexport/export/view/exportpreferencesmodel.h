...
#include "importexport/export/internal/exportrecipecontroller.h"

namespace au::importexport {
class ExportPreferencesModel : public QObject, public muse::async::Asyncable, public muse::Contextable, public IRecipeExportState
{
    Q_OBJECT
    ...
    muse::GlobalInject<IExportRecipeStore> recipeStore;
    ...
    Q_PROPERTY(QString recipeError READ recipeError NOTIFY recipeErrorChanged)
...
public:
    ...
    QString recipeError() const { return m_recipeError; }
    Q_INVOKABLE void refreshRecipes();
    Q_INVOKABLE bool saveCurrentRecipe(const QString& name,const QString& replaceId=QString(),bool allowDuplicateName=false);
    Q_INVOKABLE bool applyRecipe(const QString& id);
    Q_INVOKABLE bool removeRecipe(const QString& id);
    ExportRecipe capture() const override;
    RecipeResult<ExportRecipeFormat> describe(const ExportRecipe&) const override;
    void applyValidated(const ExportRecipe&) override;
...
private:
    ...
    QString m_recipeError;
    bool showRecipeStatus(const RecipeStatus& status);
    IExporter::Options currentExportOptions() const;
};
}
