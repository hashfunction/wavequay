...
void ExporterModule::registerExports()
{
    ...
    auto globalConfiguration=globalIoc()->resolve<muse::IGlobalConfiguration>(mname);
    m_recipeStore=std::make_shared<ExportRecipeStore>(globalConfiguration->userAppDataPath().appendingComponent("export-recipes-v1.json").toQString());
    globalIoc()->registerExport<IExportRecipeStore>(mname,m_recipeStore);
}
