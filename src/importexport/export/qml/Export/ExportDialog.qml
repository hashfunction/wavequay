...
StyledDialogView {
    ...
    ExportPreferencesModel {
        ...
        onRecipeApplied: dynamicOptionsModel.reloadFromEditor()
    }

    SaveExportRecipeDialog {
        id: saveRecipeDialog
        recipeModel: exportPreferencesModel
    }
    ManageExportRecipesDialog {
        id: manageRecipesDialog
        recipeModel: exportPreferencesModel
    }
    ...
    ColumnLayout {
        ...
        ColumnLayout {
            ...
            BaseSection {
                id: recipeSection
                title: qsTrc("export", "Spoken-audio recipe")
                rowSpacing: 8
                navigation.section: root.navigationSection
                navigation.order: 1
                StyledDropdown {
                    Layout.fillWidth: true
                    model: exportPreferencesModel.recipes
                    textRole: "name"
                    valueRole: "id"
                    currentIndex: -1
                    indeterminateText: qsTrc("export", "Choose saved export settings")
                    navigation.name: "ExportRecipe"
                    navigation.panel: recipeSection.navigation
                    navigation.order: 1
                    navigation.accessible.name: qsTrc("export", "Spoken-audio export recipe")
                    onActivated: function (index, value) { exportPreferencesModel.applyRecipe(value) }
                }
                RowLayout {
                    FlatButton {
                        text: qsTrc("export", "Save recipe")
                        navigation.name: "SaveRecipe"
                        navigation.panel: recipeSection.navigation
                        navigation.order: 2
                        onClicked: { exportPreferencesModel.refreshRecipes(); saveRecipeDialog.open() }
                    }
                    FlatButton {
                        text: qsTrc("export", "Manage recipes")
                        navigation.name: "ManageRecipes"
                        navigation.panel: recipeSection.navigation
                        navigation.order: 3
                        onClicked: { exportPreferencesModel.refreshRecipes(); manageRecipesDialog.open() }
                    }
                }
                StyledTextLabel {
                    Layout.fillWidth: true
                    visible: exportPreferencesModel.recipeError.length > 0
                    text: exportPreferencesModel.recipeError
            ...
            BaseSection {
                ...
                navigation.section: root.navigationSection
                navigation.order: recipeSection.navigation.order + 1
